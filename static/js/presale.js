    ...
    if (equipment_id != ""){
        ...
        xhr.onload=function(){
            if (this.status===200){
                ...
                ec.querySelector("#hardware_serial").textContent=data.fields.hardware_serial
                ec.querySelector("#software_serial").textContent=data.fields.software_serial
...
function inbound(e){
    warehouse_id=document.getElementById("id_warehouse").value
    equipment_id=document.getElementById("id_equipment").value
    amount = document.getElementById("id_amount").value
    if (equipment_id && warehouse_id){
        xhr=new XMLHttpRequest()
        xhr.responseType = 'blob';
        xhr.open('GET',`presale/inbound-file/${equipment_id}/${warehouse_id}/${amount}`,true)
        xhr.onload=function(){
...
function checkDevice(e){
    ...
    const mainContent=document.getElementById("main-content") 
    xhr=new XMLHttpRequest()
    xhr.open('GET',`presale/device-update/${deviceId}/`,true)
    xhr.onload=function(){
        if (this.status===200){
            const responseText=JSON.parse(this.responseText)
            content=responseText.content
